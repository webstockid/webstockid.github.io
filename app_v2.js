...
/*
 * STOCK ID SCREENER V2 — ENHANCEMENT LAYER
 * ------------------------------------------------------------
 * Prinsip:
 * 1. Seluruh fungsi/logika app.js asli tetap dipertahankan.
 * 2. Modul ini hanya menambah UX, keyboard shortcut, refresh,
 *    status pasar, dan visual hook.
 * 3. Semua nama internal memakai prefix SIDV2 agar minim bentrok.
 */

(() => {
	const SIDV2 = {
		toolbarId: 'stockidV2Toolbar',
		clockTimer: null,
		previousTicker: null,
		...
		addVisualHooks() {
			...
			main.querySelectorAll(':scope > section, :scope > div').forEach((element) => {
				if (
					...
					element.id === 'cuanCelebration' ||
					element.id === 'lossCelebration'
				) {
					return;
				}
				...
				const hasPanelShape =
					...
					element.classList.contains('glassmorphism') ||
					element.querySelector('.border-slate-800');

				if (hasPanelShape) {
					element.classList.add('v2-panel');
				}
			});

			document.querySelectorAll(
				'main h2, main h3, main [data-section-title]'
			).forEach((heading) => {
				heading.classList.add('v2-section-title');
			});
		},
		...
		buildToolbar() {
			...
			toolbar.innerHTML = `
				<div class="v2-toolbar-meta">
					<span id="stockidV2MarketDot" class="v2-live-dot"></span>
					<span id="stockidV2MarketText">Memeriksa status market...</span>
					<span class="v2-shortcut">/ fokus</span>
				</div>
				<div class="flex items-center gap-1.5">
					<span id="stockidV2Clock" class="hidden sm:inline text-[9px] text-slate-500 font-bold"></span>
					<button id="stockidV2Refresh" type="button" class="v2-refresh" title="Refresh saham aktif">
			...
			searchContainer.parentElement.appendChild(toolbar);

			const refreshButton = document.getElementById('stockidV2Refresh');
			if (refreshButton) {
				refreshButton.addEventListener('click', () => this.refreshCurrentStock());
			}
		...
		updateMarketUI() {
			const dot = document.getElementById('stockidV2MarketDot');
			const label = document.getElementById('stockidV2MarketText');
			const clock = document.getElementById('stockidV2Clock');
			...
			try {
				marketOpen = typeof isMarketOpen === 'function' && isMarketOpen();
			} catch (error) {
				marketOpen = false;
			}

			dot.classList.toggle('open', marketOpen);
			label.textContent = marketOpen ? 'Market Live • IDX' : 'Market Closed • IDX';
			...
			const refreshButton = document.getElementById('stockidV2Refresh');
			if (refreshButton) {
				refreshButton.title = marketOpen
					? 'Ambil data terbaru saham aktif'
					: 'Refresh data saham aktif';
			}
		},

		setRefreshState(isLoading) {
			const button = document.getElementById('stockidV2Refresh');
			if (!button) return;
			...
			button.innerHTML = isLoading
				? '<i class="fa-solid fa-spinner fa-spin text-[9px]"></i><span>Memuat...</span>'
				: '<i class="fa-solid fa-rotate text-[9px]"></i><span>Refresh</span>';
		},

		async refreshCurrentStock() {
			if (typeof currentTicker === 'undefined' || !currentTicker) return;

			this.setRefreshState(true);

			try {
				const ticker = String(currentTicker).trim().toUpperCase();

				if (typeof cleanExpiredCache === 'function') {
					cleanExpiredCache();
				}

				if (typeof generateAISignal === 'function') {
					await generateAISignal(ticker, true);
				}

				if (typeof fetchRealtimeFundamentals === 'function') {
				...
				if (typeof fetchCorporateAction === 'function') {
					fetchCorporateAction(ticker);
				}

				if (typeof renderAllAlerts === 'function') {
					renderAllAlerts();
				}

				if (typeof showToast === 'function') {
					showToast(`Data $${ticker} berhasil diminta ulang.`, 'success');
				}
			} catch (error) {
				console.warn('Stock ID V2 refresh error:', error);

				if (typeof showToast === 'function') {
					showToast('Refresh gagal. Silakan coba kembali.', 'error');
				}
			} finally {
				window.setTimeout(() => this.setRefreshState(false), 700);
			}
		},
		...
		setupTickerWatcher() {
			window.setInterval(() => {
				if (
					typeof currentTicker !== 'undefined' &&
					currentTicker !== this.previousTicker
				) {
					this.previousTicker = currentTicker;
					this.updateMarketUI();
				}
			}, 1000);
		},
